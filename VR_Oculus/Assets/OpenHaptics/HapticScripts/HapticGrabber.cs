...
public class HapticGrabber : MonoBehaviour 
{
	...
	private void hapticTouchEvent( bool isTouch )
	{
		...
		if( physicsToggleStyle == PhysicsToggleStyle.onTouch )
		{
            ...
            hapticDevice.GetComponent<HapticPlugin>().PhysicsManipulationEnabled = isTouch;  // [wb]: Here toggle the PhysicsManipulationEnabled.
	...
	void OnCollisionEnter(Collision collisionInfo)
	{
		...
		touching = that; //[wb]: This is the object that is currently being touched.
	}
	...
	void grab()
	{
		...
		while (body == null)
		{
			//Debug.Log("Grabbing : " + grabbing.name + " Has no body. Finding Parent. ");
    ...
    //[wb]: Get the name of the currently touched object
    public string getCurrentlyTouchedObject()
    {
        if (touching != null)
        {
            return touching.name;
        }
        else
        {
            return null;
    ...
    //June24_added
    public string getCurrentlyTouchedObject_position()
    {
        if (touching != null)
        {
            return touching.transform.position.ToString("F5");
        }
        else
        {
            return new Vector3(-1111.1f, -1111.1f, -1111.1f).ToString("F1");
        }
    }

    //June24_added
    public string getCurrentlyTouchedObject_rotation()
    {
        if (touching != null)
        {
            return touching.transform.rotation.ToString("F5");
        }
        else
        {
            return new Vector3(-1111.1f, -1111.1f, -1111.1f).ToString("F1");
