...
public class HapticPlugin : MonoBehaviour  {
	...
	// Force output
	[DllImport("OHToUnityBridge")] public static extern void setForce(string configName, double[] lateral3, double[] torque3); //!< Adds an additional force to the haptic device. Can be used for scripted forces, but in most cases using an Effect is preferable. 
    ...
    public Vector3 current_stylusPositionRaw;
    public Vector3 current_stylusVelocityRaw;
    public Vector3 current_proxyPositionRaw;
    public Vector3 current_stylusPositionWorld;
    public Quaternion current_proxyOrientationRaw;
    public Quaternion current_stylusRotationWorld;
	...
	{
		bool success = false;
		hHD = initDevice (configName);  //[wb]: DLL imported function
		if (hHD < 0)
		{
			//Error.
	...
	void FixedUpdate() 
	{
        ...
        {
            if (!(string.IsNullOrEmpty(GameObject.Find("GameSetting").GetComponent<GameSetting_ExperimentPage>().outputFullPath_currentTrial)))
            {
                System.IO.StreamWriter currentExperiment;


                if (!System.IO.File.Exists(GameObject.Find("GameSetting").GetComponent<GameSetting_ExperimentPage>().outputFullPath_currentTrial))
                {
                    currentExperiment = System.IO.File.CreateText(GameObject.Find("GameSetting").GetComponent<GameSetting_ExperimentPage>().outputFullPath_currentTrial);
                    timer = 0.0f;
                    ...
                    currentExperiment.WriteLine(firstLine);
                }
                else
                {
                    currentExperiment = new System.IO.StreamWriter(GameObject.Find("GameSetting").GetComponent<GameSetting_ExperimentPage>().outputFullPath_currentTrial, append: true);
                }

                dataToAppend = current_stylusPositionRaw + " " + 
                    current_stylusVelocityRaw + " " +
                    proxyOrientationRaw + " " +
                    proxyPositionRaw + " " +                   
                    current_stylusPositionWorld + " " +
                    stylusRotationWorld + " " +
                    stylusVelocityWorld + " " +
                    ...
                    GameObject.Find("Slider").GetComponent<CUI_ChangeValueOnHold_exp>().total_rating + " " +
                    GameObject.Find("Grabber").GetComponent<HapticGrabber>().isGrabbing() + " " +
                    GameObject.Find("Grabber").GetComponent<HapticGrabber>().isPressedButton() + " " +
                    GameObject.Find("Grabber").GetComponent<HapticGrabber>().getCurrentlyTouchedObject() + " " +
                    GameObject.Find("Grabber").GetComponent<HapticGrabber>().getCurrentlyTouchedObject_position() + " " +
                    GameObject.Find("Grabber").GetComponent<HapticGrabber>().getCurrentlyTouchedObject_rotation() + " " +
                    GameObject.Find("Sphere").transform.position.ToString("F5") +" " +
                    GameObject.Find("Sphere").transform.rotation.ToString("F5") + " " +
                    GameObject.Find("Cube").transform.GetComponent<Rigidbody>().mass +
                    GameObject.Find("Cube").transform.GetComponent<Rigidbody>().transform.localScale +
                    GameObject.Find("Cube").transform.parent.name + " " +
                    timer;


                currentExperiment.WriteLine(dataToAppend);
	...
	private void updateDevice()
	{
        ...
        if (SceneManager.GetActiveScene().buildIndex == 3)
        {
            ...
            current_stylusPositionRaw = new Vector3();
            current_stylusPositionRaw = stylusPositionRaw;
            current_stylusVelocityRaw = new Vector3();
            current_stylusVelocityRaw = stylusVelocityRaw;

            current_proxyPositionRaw = new Vector3();
            current_stylusPositionRaw = proxyPositionRaw;

            Quaternion current_proxyOrientationRaw = new Quaternion();
            current_proxyOrientationRaw = proxyOrientationRaw;

            current_stylusPositionWorld = new Vector3();
            current_stylusPositionWorld = stylusPositionWorld;

            Quaternion current_stylusRotationWorld = new Quaternion();
            current_stylusRotationWorld = stylusRotationWorld;
        }
	...
	private void updateManipulator()
	{
...

		{
			...
			force *= body.mass;  //[wb]: This is not affected by the mass value in the rigid body
            force *= 1000;
